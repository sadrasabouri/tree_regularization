...
class TransformerResult(DotDict):
    ...
    @staticmethod
    def create(data: torch.Tensor, length: torch.Tensor, hidden_states: Optional[torch.Tensor]):
        return TransformerResult({"data": data, "length": length, "hidden_states": hidden_states})
...
class TransformerLM(torch.nn.Module):
    ...
    def run_teacher_forcing(
        ...
        res, hidden_states = self.trafo(src, tgt=None, src_length_mask=in_len_mask, get_hidden_states=get_hidden_states, layer_id=layer_id, sci_heads=sci_heads)

        if self.output_dropout is not None:
            res = self.output_dropout(res)
        
        return TransformerResult.create(self.output_map(res), src_len, hidden_states)
    ...
    def forward(
        ...
        return self.run_teacher_forcing(src, src_len, get_hidden_states, layer_id, sci_heads)
