        ...
        if tgt is None:
            ...
            if get_hidden_states:
                hidden_states =  self.encoder(
                                    src, src_length_mask=src_length_mask, pos_mask=pos_mask, get_all_layers=True, **kwargs
                                )
                proportion = int(sci_heads * hidden_states[0].shape[-1])
                return hidden_states[-1], hidden_states[layer_id][:, :, :proportion]
            else:
                return self.encoder(
            ...
            return self.decoder(tgt, memory, tgt_mask, src_length_mask), None
